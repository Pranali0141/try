	...
	public UserDetails signIn(Users user) {
		...
		try {
			if (done) {
				...
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/rcitydb","root", "");
				Statement st = con.createStatement();
				String query = "select fname,mname,lname from users where userId = (SELECT userid FROM login_details WHERE userName = '" + user.getUserName()+ "' AND Password = '" + user.getPassword() + "')";
